...
const ProductDetails = ({ slug, category, collection, initialDetails }: Props) => {
  const { data, isLoading } = useProductBySlug(category, collection, slug, initialDetails);

  const product = data ?? initialDetails;
  ...
  // Prefer the images array for the gallery; fall back to the single thumbnail if images is empty
  const gallery = details.images?.length ? details.images : [details.thumbnail];
  const activeSrc = gallery[activeImage] ?? details.thumbnail;
  ...
  // No per-size stock is returned by the API, so treat all listed sizes as available
  // unless overall stock is 0.
  const outOfStock = details.stock <= 0;

  const quickInfoTags = [
    details.brand,
    details.colors?.length ? `${details.colors.length} colour${details.colors.length > 1 ? 's' : ''}` : null,
    outOfStock ? 'Out of stock' : `${details.stock} in stock`,
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="max-w-[1450px] mx-auto px-6 py-3 text-sm text-gray-500">
        ...
        <span className="hover:text-gray-800 cursor-pointer transition-colors capitalize">
          {category}
        </span>
        <span className="mx-2 text-gray-300">/</span>
        <span className="hover:text-gray-800 cursor-pointer transition-colors capitalize">
      ...
      <div className="max-w-[1450px] mx-auto px-6 py-4">
        <div className="grid grid-cols-12 gap-10">
          ...
          <div className="col-span-12 lg:col-span-5 pt-2">
            ...
            {details.description && (
              <>
                <p className="text-sm text-gray-600 leading-relaxed mb-6">{details.description}</p>
                <hr className="border-gray-100 mb-6" />
              </>
            )}
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex items-center border border-gray-300 rounded-md overflow-hidden">
                ...
                <button
                  onClick={() => setQuantity((q) => Math.min(details.stock || q + 1, q + 1))}
                  disabled={outOfStock}
                  className="w-9 h-9 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition-colors text-lg font-light disabled:opacity-40"
                >
                  +
                </button>
              </div>
            ...
            {/* Quick Info Pills */}
            {quickInfoTags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {quickInfoTags.map((tag) => (
                  <span
                    key={tag}
