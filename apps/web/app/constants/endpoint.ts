import { getproduct } from '../services/productapi/productapi';

export const endpoints = {
  auth: {
    signup: '/auth/signup',
    signin: '/auth/signin',
    refresh: '/auth/refresh',
    logout: '/auth/logout',
  },
  category: {
    createcategory: '/admincate/categories',
    getcategory: (id: string | number) => `/usercate/categories/${id}`,
    getallcategory: '/usercate/categories/',
    updatecategory: (id: string | number) => `/admincate/categories/${id}`,
    deletecategory: (id: string | number) => `/admincate/categories/${id}`,
  },
  subcategory: {
    createsubcategory: '/admincate/subcategories',
    getsubcategory: (id: string | number) => `/usercate/subcategories/${id}`,
    getallsubcategory: '/usercate/subcategories',
    updatesubcategory: (id: string | number) => `/admincate/subcategories/${id}`,
    deletesubcategory: (id: string | number) => `/admincate/subcategories/${id}`,
  },
  collection: {
    createcollection: '/admincate/collections',
    getallcollection: '/usercate/collections',
    getProductsByCollection: '/usercate/subproduct',
    getcollection: (id: string | number) => `/usercate/collections/${id}`,
    updatecollection: (id: string | number) => `/admincate/collections/${id}`,
    deletecollection: (id: string | number) => `/admincate/collections/${id}`,
  },
  product: {
    createproduct: '/admincate/products',
    getallproducts: '/usercate/products',
    getproductbyslug: '/usercate/productbyslug',
    getproduct: (id: string | number) => `/usercate/products/${id}`,
    updateproduct: (id: string | number) => `/admincate/products/${id}`,
    deleteproduct: (id: string | number) => `/admincate/products/${id}`,
  },
  homepage: {
    homecollection: '/homecate/homedata/categories',
    homenewarived: '/homecate/homedata/newarrival',
    homespotlight: '/homecate/homedata/sportlight',
  },
  carts: {
    createcart: (productId: string) => `/cart/create/${productId}`,
    getcart: '/cart/get',
    countcart: '/cart/count',
    deletecart: (productId: string) => `/cart/delete/${productId}`,
    clearcart: '/cart/clear',
    updatecart: (productId: string) => `/cart/update/${productId}`,
  },

  wishlists: {
    create: (productId: string) => `/wishlist/create/${productId}`,
    get: '/wishlist/get',
    movetocart: (productId: string) => `/wishlist/movetocart/${productId}`,
    delete: (productId: string) => `/wishlist/delete/${productId}`,
  },
} as const;
